...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("products").select("*").limit(300);
  ...
  const filteredProducts = products
    .filter(p => {
      const title = p.title || p.name_en || "";
      return title.toLowerCase().includes(searchQuery.toLowerCase());
    })
    .sort((a, b) => {
      const nameA = a.title || a.name_en || "";
      const nameB = b.title || b.name_en || "";
            ...
            <tbody className="text-pure-white divide-y divide-gray-800">
              {loading ? (
                ...
                paginatedProducts.map((product) => {
                  const title = product.title || product.name_en || product.slug;
                  return (
